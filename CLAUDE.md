...
Five modules in `omni_route_config/`. Each owns one concern; combined they implement the bootstrap → configure → use loop.
...
- `docker` — `docker run --rm -d ... diegosouzapw/omniroute:latest`.

Health probes hit `/api/health`, `/api/version`, `/` in that order. `tear_down()` only kills processes spawned by THIS package (PID file gated) — externally-started instances are never touched.
...
The CLI exit codes map to this: `0` = ok, `2` = unreachable, `3` = some apply errors, `4` = client extra missing, `5` = smoke-test exception. Don't break that mapping.
...
`config/free-providers.yaml` is shipped via `setuptools.package-data` (see `pyproject.toml`). When adding a provider:
1. Confirm the `provider` id exists in OmniRoute upstream.
